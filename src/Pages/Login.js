  ...
  const submitForm = (e) => {
    ...
    axios.post("http://localhost/back2/login.php", sendData).then((result) => {
      if (result.data.first_name) {
        // ça permet de vérifier si l'utilisateur existe ou non dans la base de données et si le mot de passe est correct ou non
        window.localStorage.setItem("user", JSON.stringify(result.data));
        setAuth(result.data); // ça permet de stocker les données de l'utilisateur connecté dans le state auth
